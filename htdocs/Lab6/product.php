...
function spudetail($db_con, $spu){
    ...
    if($query->fetchColumn()>0){
        ...
        foreach($info_array as $info){
            echo <<<EOT
                    <div class="row">
                        ...
                        <div class="col-lg-9 col-mg-9 col-sm-9 btn-group btn-group-justified" data-toggle="buttons" id="info_{$i}">
            EOT;
            ...
            foreach($sku_details as $sku_row){
                echo <<<EOT
                    <label class="btn btn-primary">
                        <input type="radio" class="toggle" name="info_{$i}" value="{$sku_row["info_".$i]}"> {$sku_row["info_".$i]}
                    </label>
                EOT;
            }
            $i++;
        ...
        echo <<<EOT
                        ...
                        <div class="row">
                            <p id="price">$ xxxx</p>
                            <input type="hidden" id="spu" value="{$spu}" >
                            <label class="btn btn-primary" id="addSku">Add to Cart</label>
                        </div>
        EOT;
    } else {
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $(".btn-group").on('change', function(){
                ...
                var info_1 = $('#info_1 input:radio:checked').val();
                var info_2 = $('#info_2 input:radio:checked').val();
                if(info_1 && info_2) {
                    var url = "sku.php?spu=" + spu + "&info_1=" + info_1 + "&info_2=" + info_2;
                    $.get(url,function(result){
                        if(result.code == 200){
                            document.getElementById("price").innerHTML = "$: " + result.data.price;
                            document.getElementById("addSku").onclick = function(){window.open("cart.php?sku=" + result.data.sku); };
                        } else {
