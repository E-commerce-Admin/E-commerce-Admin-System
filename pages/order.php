        ...
        <div class="col-12">
          <div class="card mb-4">
            <div class="card-header pb-0">
              ...
              <h6>Order</h6>
              <a>Please note that as an admin, you are
                only authorized to view the order information. You are strictly prohibited from creating, modifying, or
                deleting
                any order information as it may lead to ethical issues. Please contact the associated customers to
                modify the orders on their side.</a>
              <h6></h6>
            ...
            <div class="d-flex justify-content-between">
              <div class="form-group">
                ...
                <select id="sort-by" name="sort-by" class="form-control" onchange="location = this.value;">
                  ...
                  <option value="?sort-by=id_coupon" <?php if (isset($_GET['sort-by']) && $_GET['sort-by'] == 'id_coupon')
                    echo 'selected'; ?>>Coupon ID</option>
                ...
                <table class="table align-items-center mb-0">
                  <thead>
                    <tr class="text-xs font-weight-bold opacity-6">
                      ...
                      <th class="align-middle text-left">Coupon ID</th>
                  ...
                  <tbody>
                    ...
                    $allowed_sort_fields = ['id_order', 'order_date', 'customer_name', 'product_name', 'quantity', 'price', 'id_coupon', 'subtotal', 'shipping_address', 'payment_method'];
                    $sort_by = in_array($sort_by, $allowed_sort_fields) ? $sort_by : 'id_order';

                    $search_term = $_GET['search'] ?? '';

                    $qry_order = mysqli_query($conn, "SELECT `order`.id_order, `order`.create_time AS order_date, `order`.user_name AS customer_name, `order`.product_name, `order`.quantity, `order`.price, `order`.id_coupon, `order`.subtotal, `order`.shipping_address, `order`.payment_method
FROM `order`
WHERE (`order`.id_order LIKE '%$search_term%' OR `order`.create_time LIKE '%$search_term%' OR `order`.user_name LIKE '%$search_term%' OR `order`.product_name LIKE '%$search_term%' OR `order`.quantity LIKE '%$search_term%' OR `order`.price LIKE '%$search_term%' OR `order`.id_coupon LIKE '%$search_term%' OR `order`.subtotal LIKE '%$search_term%' OR `order`.shipping_address LIKE '%$search_term%' OR `order`.payment_method LIKE '%$search_term%')
